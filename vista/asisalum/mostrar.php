<?php
	...
	if(isset($_GET['filter'])){
		$filter = $_GET['filter'];
		$datos = $database->query("SELECT alumnos.codalum, alumnos.dnia, alumnos.nombrea, alumnos.apellidoa, padres.nompadr,  grado.nomgra, clase.nomclase, seccion.nomsec, 
	...
	FROM alumnos INNER JOIN padres ON alumnos.codpad = padres.codpad INNER JOIN grado ON alumnos.codgra=grado.codgra INNER JOIN clase ON alumnos.codclase= clase.codclase INNER JOIN seccion ON alumnos.codsec=seccion.codsec
		WHERE grado.codgra='$filter' ORDER BY codalum ASC")->fetchAll(PDO::FETCH_OBJ);					
	}else{
		$datos = $database->query("SELECT alumnos.codalum, alumnos.dnia, alumnos.nombrea, alumnos.apellidoa, padres.nompadr,  grado.nomgra, clase.nomclase, seccion.nomsec, 
	alumnos.fecha_naci, alumnos.genero, alumnos.grupo_san, alumnos.direccion, alumnos.pais,alumnos.foto, alumnos.usualu, alumnos.password,alumnos.role,alumnos.fechare ,alumnos.estado, alumnos.fecha_create 
	FROM alumnos INNER JOIN padres ON alumnos.codpad = padres.codpad INNER JOIN grado ON alumnos.codgra=grado.codgra INNER JOIN clase ON alumnos.codclase= clase.codclase INNER JOIN seccion ON alumnos.codsec=seccion.codsec ORDER BY codalum ASC")->fetchAll(PDO::FETCH_OBJ);
	}
?>
<!DOCTYPE html>
<html lang="es">
            ...
            <ul class="nav nav-tabs nav-justified"  style="font-size: 17px;">
              ...
              <li role="presentation"><a href="seccion.php">Secciones</a></li>
			   <li role="presentation"><a href="plan.php">Plan </a></li>
			    <li role="presentation"><a href="asignacion.php">Asignación</a></li>
		...
		<div class="row">
			<div class="col-xs-12 lead">
				<ol class="breadcrumb">
				  ...
				  <li><a href="../vista/asisdoc/mostrar.php">Asistencia de profesores</a></li>
				</ol>
			</div>
		</div>
	</div>
...
<div class="input-group mb-5" style="margin-right: -10em; min-width: 1000px;">
	...
	<hr>
		...
		<p>Control de asistencia de  <strong>Alumnos</strong></p>
			...
			try{	
				...
				foreach ($db->query($sql) as $row) {
					...
					<tr>
						...
						<td>
		<a href="#edit_<?php echo $row['codasis']; ?>" title="Editar" data-backdrop="false" class="btn btn-primary btn-sm" data-toggle="modal"><span class="glyphicon glyphicon-edit"></span></a>
						<?php include('../vista/asisalum/BorrarEditarModal.php'); ?>		
						</td>
	...
	<script type="text/javascript">
		...
		$(document).on('change', '#asistencia', function(){
			...
			if (datoasistencia != "0") {
				$.ajax({
					url: '../vista/asisalum/enviardatos2.php',
					type: 'POST',
					data: {op: 'tomarasistencia', datoasistencia: datoasistencia, codalum: codalum, fechare: fechare}
				}).done((res) => {
